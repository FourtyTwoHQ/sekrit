...
class ConfigTest < Minitest::Test
  ...
  def setup
    @config = Sekrit::Config.new(path: File.join(__dir__, 'Sekritfile'))
  end
  ...
  def test_it_loads_bundles_property
    ...
    assert_equal expected.sort, @config.bundles.map { |b| b.id }.sort
  end
  ...
  def test_it_loads_shared_files_files_property
    refute_nil @config.shared_files.files
    assert @config.shared_files.files.empty?
  end
